...
function dt_breadcrumb( $args = array() ) {
	...
	$defaults = array(
		'separator' => '/',
		'before' => '<span class="breadcrumb-title">' . __( 'Browse:', 'engine' ) . '</span>',
		'after' => false,
		'front_page' => true,
		'show_home' => __( 'Home', 'engine'  ),
		'echo' => true
	);
...
function breadcrumb_trail_get_items( $args = array() ) {
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'engine' ) );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'engine' ), get_the_time( __( 'i', 'engine' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a', 'engine' ) );
		}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'engine' ) ) . '">' . get_the_time( __( 'Y', 'engine' ) ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'engine' ) ) . '">' . get_the_time( __( 'F', 'engine' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'd', 'engine' ) );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'engine' ) ) . '">' . get_the_time( __( 'Y', 'engine' ) ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'engine' ), get_the_time( esc_attr__( 'W', 'engine' ) ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'engine' ) ) . '">' . get_the_time( __( 'Y', 'engine' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'engine' ) );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'engine' ) );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'engine' ), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found', 'engine' );
...
function breadcrumb_trail_map_rewrite_tags( $post_id = '', $path = '', $args = array() ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( '%year%' == $tag )
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'engine' ), $post_id ) . '">' . get_the_time( __( 'Y', 'engine' ), $post_id ) . '</a>';

			/* If using the %monthnum% tag, add a link to the monthly archive. */
			elseif ( '%monthnum%' == $tag )
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'engine' ), $post_id ) . '">' . get_the_time( __( 'F', 'engine' ), $post_id ) . '</a>';

			/* If using the %day% tag, add a link to the daily archive. */
			elseif ( '%day%' == $tag )
				$trail[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'engine' ), $post_id ) . '">' . get_the_time( __( 'd', 'engine' ), $post_id ) . '</a>';
